BEGIN
     ...
     DECLARE VS_EMAILS, VS_EMAILS_DEPTO, VS_EMAILS_USERCLI, VS_ASSUNTO, VS_CORPO_TITULO, VS_OCORREN_PARA VARCHAR(1000) DEFAULT '';
  	DECLARE VS_CORPO_TEXTO, VS_CORPO_TEXTO_NOVO VARCHAR(2000) DEFAULT '';    
...
*/
  		...
  		IF (TRIM(VS_EMAILS)) > '' THEN
  			...
  			SELECT 
			(CONCAT('Cliente: ',C.CLIENTE,'-',TRIM(U.NOME),'<br>','Usuário Criador: ', COALESCE(TRIM(CRIADOR.NOME),''),'<br>','Usuário Responsável: ', COALESCE(TRIM(RESPON.NOME),''),'<br>','Titulo: ', trim(c.titulo),'<br>','Sistema: ', COALESCE(trim(sistemas.nome),''),'<br>','<br>','<br> ')) CORPO_TITULO
            ...
            SET VS_CORPO_TEXTO_NOVO = CONCAT(VS_CORPO_TITULO, CHAR(13), CHAR(10), VS_CORPO_TEXTO);
 			...
 			INSERT INTO `gvmsistemas`.`email` (`remetente`, `dests`, `assunto`, `data_criacao`,`corpo_titulo`,`corpo_texto`,`id_chamado`) VALUES
			  ...
			  INSERT INTO `gvmsistemas_emails`.`emails` (`remetente`, `dests`, `assunto`, `data_criacao`,`corpo_texto`,`id_chamado`,`attemps`, `enviado`, `retornado`) VALUES
  			('chamados@gvmsistemas.com.br',VS_EMAILS,VS_ASSUNTO,now(),VS_CORPO_TEXTO_NOVO,NEW.ID, 0, false, false);
